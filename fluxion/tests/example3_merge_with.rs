...
async fn test_merge_with_repository_pattern() -> anyhow::Result<()> {
    ...
    tx_users.try_send(user_created1)?;
    tx_users.try_send(user_created2)?;
    tx_orders.try_send(order_placed1)?;
    tx_payments.try_send(payment_received1)?;
    tx_orders.try_send(order_placed2)?;
