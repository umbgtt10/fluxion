...
async fn test_functional_take_while_with() -> anyhow::Result<()> {
    // Arrange
    let (source_tx, source_stream) = test_channel::<Timestamped<TestData>>();
    let (predicate_tx, predicate_stream) = test_channel::<Timestamped<TestData>>();
    ...
    let bob = Timestamped::new(person_bob());
    let charlie = Timestamped::new(person_charlie());
...
async fn test_functional_with_latest_from() -> anyhow::Result<()> {
    // Arrange
    let (primary_tx, primary_stream) = test_channel::<Timestamped<TestData>>();
    let (secondary_tx, secondary_stream) = test_channel::<Timestamped<TestData>>();
    ...
    primary_tx.send(Timestamped::new(person_alice()))?;
