...
use fluxion_stream_multi::{CombineLatestExt, MapOrderedExt, OrderedStreamExt, ScanOrderedExt};
...
async fn test_combine_latest_scan_ordered_total_age() -> anyhow::Result<()> {
    ...
    let combine_filter = |_: &CombinedState<TestData, u64>| true;

    let mut stream = person_stream
        .combine_latest(vec![animal_stream], combine_filter)
        .scan_ordered(
            0u32,
            |total_age: &mut u32, state: &CombinedState<TestData, u64>| {
