...
use fluxion_stream_multi::merge_with::MergedStream;
...
async fn subject_combined_with_stream_via_combine_latest() -> anyhow::Result<()> {
    ...
    let mut combined = primary_rx.combine_latest(vec![gate_subject.subscribe().unwrap()], |_| true);
...
async fn subject_as_filter_for_emit_when() -> anyhow::Result<()> {
    ...
    let mut stream = src_rx.emit_when(
...
async fn subject_take_while_with_stops_on_short_plants() -> anyhow::Result<()> {
    ...
    let mut stream = src_rx.take_while_with(
