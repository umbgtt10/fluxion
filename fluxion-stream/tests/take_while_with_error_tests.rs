...
async fn test_take_while_with_stops_on_false_despite_errors() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(0, 3)))?;
...
async fn test_take_while_with_source_before_filter() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(50, 2)))?;
...
async fn test_take_while_with_filter_updates_then_source() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(100, 1)))?;
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(50, 2)))?;
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(25, 3)))?;
...
async fn test_take_while_with_already_terminated() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(0, 3)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(2, 4)))?;
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 5)))?;
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(100, 6)))?;
...
async fn test_take_while_with_filter_false_on_first_source() -> anyhow::Result<()> {
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(100, 2)))?;
...
async fn test_take_while_with_alternating_filter_values() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(50, 3)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(2, 4)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(25, 5)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 6)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
...
async fn test_take_while_with_error_recovery() -> anyhow::Result<()> {
    ...
    // Continue
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 4)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
