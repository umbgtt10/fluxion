...
async fn test_with_latest_from_propagates_primary_error() -> anyhow::Result<()> {
    ...
    secondary_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 1)))?;
    assert_no_element_emitted(&mut result, 100).await;
    primary_tx.send(StreamItem::Value(Sequenced::with_timestamp(1, 2)))?;
    assert!(matches!(
    ...
    primary_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 4)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
...
async fn test_with_latest_from_propagates_secondary_error() -> anyhow::Result<()> {
    ...
    secondary_tx.send(StreamItem::Value(Sequenced::with_timestamp(30, 5)))?;
    assert_no_element_emitted(&mut result, 100).await;
    primary_tx.send(StreamItem::Value(Sequenced::with_timestamp(2, 6)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
...
async fn test_with_latest_from_selector_continues_after_error() -> anyhow::Result<()> {
    ...
    secondary_tx.send(StreamItem::Value(Sequenced::with_timestamp(100, 1)))?;
    assert_no_element_emitted(&mut result, 100).await;
    primary_tx.send(StreamItem::Value(Sequenced::with_timestamp(1, 2)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
    ...
    // Update secondary
    secondary_tx.send(StreamItem::Value(Sequenced::with_timestamp(200, 4)))?;
    assert_no_element_emitted(&mut result, 100).await;
    primary_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 5)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
