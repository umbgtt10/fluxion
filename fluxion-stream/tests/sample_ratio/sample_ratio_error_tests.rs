...
async fn test_sample_ratio_multiple_errors_pass_through() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Error(error1))?;
    tx.send(StreamItem::Error(error2))?;
    tx.send(StreamItem::Error(error3))?;
...
async fn test_sample_ratio_error_on_empty_stream() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Error(FluxionError::stream_error("lone error")))?;
