...
async fn test_emit_when_propagates_source_error() -> anyhow::Result<()> {
    ...
    // Act
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(15, 1)))?;

    // Send source value
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 2)))?;
    ...
    // Continue with value
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(30, 3)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
...
async fn test_emit_when_propagates_filter_error() -> anyhow::Result<()> {
    ...
    // Act
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(5, 1)))?;
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(15, 4)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(20, 3)))?;
...
async fn test_emit_when_predicate_continues_after_error() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(25, 4)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(30, 5)))?;
...
async fn test_emit_when_error_before_filter_ready() -> anyhow::Result<()> {
    ...
    // Continue
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(5, 2)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 1)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
...
async fn test_emit_when_filter_returns_false_on_source_update() -> anyhow::Result<()> {
    ...
    // Setup: filter value
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(50, 1)))?;

    // Source value less than filter - should NOT emit
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 2)))?;

    // Send another source that passes
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(100, 3)))?;
...
async fn test_emit_when_filter_returns_false_on_filter_update() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(20, 3)))?;

    // Update filter back to passing: filter=8 (passes: 10 > 8 = true)
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(8, 4)))?;
...
async fn test_emit_when_multiple_filter_updates_no_source() -> anyhow::Result<()> {
    ...
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(5, 1)))?;
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 2)))?;
    filter_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 3)))?;

    // Now send source value - should use latest filter (3)
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(15, 4)))?;
...
async fn test_emit_when_alternating_false_conditions() -> anyhow::Result<()> {
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(20, 4)))?;
