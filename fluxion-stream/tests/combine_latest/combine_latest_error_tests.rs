...
async fn test_combine_latest_propagates_error_from_primary_stream() -> anyhow::Result<()> {
    ...
    tx2.send(StreamItem::Value(Sequenced::with_timestamp(20, 4)))?;
...
async fn test_combine_latest_propagates_error_from_secondary_stream() -> anyhow::Result<()> {
    ...
    // Send initial values
    tx1.send(StreamItem::Value(Sequenced::with_timestamp(1, 1)))?;
    tx2.send(StreamItem::Value(Sequenced::with_timestamp(10, 3)))?;
    ...
    // Continue with more values
    tx2.send(StreamItem::Value(Sequenced::with_timestamp(30, 5)))?;
...
async fn test_combine_latest_filter_predicate_continues_after_error() -> anyhow::Result<()> {
    ...
    tx1.send(StreamItem::Value(Sequenced::with_timestamp(0, 3)))?;
