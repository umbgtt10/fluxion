...
async fn test_ordered_merge_timestamp_ordering() -> anyhow::Result<()> {
    ...
    tx1.send(Sequenced::with_timestamp(person_charlie(), 3))?;
    tx2.send(Sequenced::with_timestamp(person_alice(), 1))?;
    tx1.send(Sequenced::with_timestamp(person_dave(), 4))?;
    tx2.send(Sequenced::with_timestamp(person_bob(), 2))?;
...
async fn test_ordered_merge_multiple_streams() -> anyhow::Result<()> {
    ...
    person_tx.send(Sequenced::with_timestamp(person_alice(), 5))?;
    animal_tx.send(Sequenced::with_timestamp(animal_dog(), 1))?;
    plant_tx.send(Sequenced::with_timestamp(plant_rose(), 3))?;
    animal_tx.send(Sequenced::with_timestamp(animal_spider(), 2))?;
    plant_tx.send(Sequenced::with_timestamp(plant_oak(), 4))?;
...
async fn test_ordered_merge_one_stream_closes_early() -> anyhow::Result<()> {
    ...
    tx1.send(Sequenced::with_timestamp(person_alice(), 1))?;
    tx1.send(Sequenced::with_timestamp(person_bob(), 2))?;
    drop(tx1);
...
async fn test_ordered_merge_duplicate_timestamps() -> anyhow::Result<()> {
    ...
    tx1.send(Sequenced::with_timestamp(person_alice(), 1))?;
    tx2.send(Sequenced::with_timestamp(animal_dog(), 1))?;
    tx1.send(Sequenced::with_timestamp(person_bob(), 2))?;
    tx2.send(Sequenced::with_timestamp(animal_spider(), 2))?;
...
async fn test_ordered_merge_large_volume() -> anyhow::Result<()> {
    ...
    for i in 0..1000 {
        tx1.send(Sequenced::with_timestamp(i * 2, (i * 2) as u64))?;
        tx2.send(Sequenced::with_timestamp(i * 2 + 1, (i * 2 + 1) as u64))?;
    }
...
async fn test_ordered_merge_parallel_sends() -> anyhow::Result<()> {
    ...
    spawn(async move {
        for i in 0..100 {
            tx1.send(Sequenced::with_timestamp(person_alice(), (i * 2) as u64))
                .unwrap();
        }
    });
    ...
    spawn(async move {
        for i in 0..100 {
            tx2.send(Sequenced::with_timestamp(animal_dog(), (i * 2 + 1) as u64))
                .unwrap();
        }
    });
...
async fn test_ordered_merge_preserves_timestamp_metadata() -> anyhow::Result<()> {
    ...
    // Act
    tx1.send(Sequenced::with_timestamp(person_alice(), 10))?;
    tx2.send(Sequenced::with_timestamp(animal_dog(), 20))?;
...
async fn test_ordered_merge_mixed_types_in_enum() -> anyhow::Result<()> {
    ...
    plant_tx.send(Sequenced::with_timestamp(plant_rose(), 3))?;
    person_tx.send(Sequenced::with_timestamp(person_bob(), 4))?;
    animal_tx.send(Sequenced::with_timestamp(animal_bird(), 5))?;
    plant_tx.send(Sequenced::with_timestamp(plant_sunflower(), 6))?;
