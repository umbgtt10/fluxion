...
async fn test_take_while_with_error_propagation_at_end_of_chain() -> anyhow::Result<()> {
    ...
    // Act & Assert
    condition_tx.try_send(StreamItem::Value(Sequenced::with_timestamp(true, 0)))?;
    condition_tx.try_send(StreamItem::Value(Sequenced::with_timestamp(false, 3)))?;
    ...
    source_tx.try_send(StreamItem::Error(FluxionError::stream_error("Error1")))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Error(_)
    ));
