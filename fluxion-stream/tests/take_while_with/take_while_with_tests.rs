...
async fn test_take_while_basic() -> anyhow::Result<()> {
    ...
    let mut result = source_stream.take_while_with(filter_stream, |filter_val| *filter_val);

    // Act & Assert
    filter_tx.send(Sequenced::new(true))?;
    assert_no_element_emitted(&mut result, 100).await;
    source_tx.send(Sequenced::new(animal_cat()))?;
    assert_eq!(extract_element(&mut result).await, animal_cat());

    source_tx.send(Sequenced::new(animal_dog()))?;
    assert_eq!(extract_element(&mut result).await, animal_dog());

    source_tx.send(Sequenced::new(person_alice()))?;
    assert_eq!(extract_element(&mut result).await, person_alice());
...
async fn test_take_while_no_filter_value() -> anyhow::Result<()> {
    ...
    source_tx.send(Sequenced::new(person_bob()))?;
    assert_eq!(extract_element(&mut result).await, person_bob());
