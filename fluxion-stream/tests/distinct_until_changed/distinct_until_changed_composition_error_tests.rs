...
async fn test_distinct_until_changed_multiple_errors_in_composition() -> anyhow::Result<()> {
    ...
    // Act & Assert
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(1, 1)))?; // 2
    assert_eq!(
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(1, 4)))?;
    assert_no_element_emitted(&mut result, 100).await;

    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(2, 5)))?;
    assert_eq!(
        unwrap_value(Some(unwrap_stream(&mut result, 100).await)).value,
        4
    );
