...
async fn test_filter_ordered_distinct_until_changed() -> anyhow::Result<()> {
    ...
    tx.try_send(Sequenced::new(person_alice()))?; // Duplicate, filtered by distinct
    ...
    tx.try_send(Sequenced::new(person_bob()))?; // Duplicate, filtered by distinct
...
async fn test_distinct_until_changed_with_map_composition() -> anyhow::Result<()> {
    ...
    tx.try_send(Sequenced::new(person_alice()))?; // Age 25 - same
    tx.try_send(Sequenced::new(person_bob()))?; // Age 30 - different
    assert_eq!(
        unwrap_value(Some(unwrap_stream(&mut result, 100).await)).value,
        30
    );

    tx.try_send(Sequenced::new(person_bob()))?; // Age 30 - same
    tx.try_send(Sequenced::new(person_bob()))?; // Age 30 - same
    tx.try_send(Sequenced::new(person_charlie()))?; // Age 35 - different
    assert_eq!(
        unwrap_value(Some(unwrap_stream(&mut result, 100).await)).value,
        35
    );
