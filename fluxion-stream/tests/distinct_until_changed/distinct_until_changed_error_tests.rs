...
async fn test_distinct_until_changed_alternating_errors_and_values() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(true, 1)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(false, 2)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(v) if !v.value
    ));
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(false, 3)))?; // Duplicate - filtered

    tx.try_send(StreamItem::Error(FluxionError::stream_error("E3")))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Error(_)
    ));

    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(true, 4)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(v) if v.value
    ));
