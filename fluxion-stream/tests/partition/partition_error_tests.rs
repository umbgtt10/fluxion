...
async fn test_partition_error_message_preserved() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Error(FluxionError::stream_error(error_msg)))?;
...
async fn test_partition_mixed_values_and_error() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Value(Sequenced::new(person_alice())))?; // person
    tx.send(StreamItem::Value(Sequenced::new(animal_dog())))?; // non-person
    tx.send(StreamItem::Value(Sequenced::new(person_bob())))?; // person
    tx.send(StreamItem::Error(FluxionError::stream_error(
        "mid-stream error",
    )))?;
    // These won't be received due to error termination
    tx.send(StreamItem::Value(Sequenced::new(animal_spider())))?;
