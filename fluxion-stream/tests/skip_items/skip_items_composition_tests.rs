...
async fn test_map_ordered_skip_items() -> anyhow::Result<()> {
    ...
    // Act
    tx.try_send(Sequenced::new(person_alice()))?; // age=25, Skipped
    tx.try_send(Sequenced::new(person_dave()))?; // age=28, Skipped
    tx.try_send(Sequenced::new(person_bob()))?; // age=30, Emitted
    tx.try_send(Sequenced::new(person_charlie()))?; // age=35, Emitted
    tx.try_send(Sequenced::new(person_diane()))?; // age=40, Emitted
...
async fn test_ordered_merge_then_skip_items() -> anyhow::Result<()> {
    ...
    s1_tx.try_send(Sequenced::new(person_charlie()))?;

    // 4. Stream 2 emits (Should be emitted)
    s2_tx.try_send(Sequenced::new(person_dave()))?;
    ...
    s1_tx.try_send(Sequenced::new(person_diane()))?;
