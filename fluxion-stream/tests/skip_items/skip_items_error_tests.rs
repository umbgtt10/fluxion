...
async fn test_skip_propagates_errors() -> anyhow::Result<()> {
    ...
    // Act
    tx.try_send(StreamItem::Value(Sequenced::new(1)))?; // Skipped
    tx.try_send(StreamItem::Error(FluxionError::stream_error("error")))?; // Skipped
    tx.try_send(StreamItem::Value(Sequenced::new(2)))?; // Emitted
    tx.try_send(StreamItem::Error(FluxionError::stream_error("error2")))?; // Emitted
...
async fn test_skip_counts_errors_as_items() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Value(Sequenced::new(1)))?; // This should be emitted
