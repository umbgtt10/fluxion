...
async fn test_filter_ordered_preserves_ordering() -> anyhow::Result<()> {
    ...
    tx.try_send(Sequenced::new(person_alice()))?; // 25 - odd, filtered
    tx.try_send(Sequenced::new(person_bob()))?; // 30 - even, kept
    tx.try_send(Sequenced::new(person_charlie()))?; // 35 - odd, filtered
    tx.try_send(Sequenced::new(person_diane()))?; // 40 - even, kept
    tx.try_send(Sequenced::new(person_dave()))?; // 28 - even, kept
...
async fn test_filter_ordered_complex_predicate() -> anyhow::Result<()> {
    ...
    tx.try_send(Sequenced::new(animal_dog()))?; // kept
    tx.try_send(Sequenced::new(plant_rose()))?; // filtered
    tx.try_send(Sequenced::new(person_diane()))?; // 40 - kept
...
async fn test_filter_ordered_with_pattern_matching() -> anyhow::Result<()> {
    ...
    // Act
    tx.try_send(Sequenced::new(person_alice()))?; // Alice - kept
    tx.try_send(Sequenced::new(person_bob()))?; // Bob - filtered
    tx.try_send(Sequenced::new(person_charlie()))?; // Charlie - filtered
    tx.try_send(Sequenced::new(person_dave()))?; // Dave - kept
    tx.try_send(Sequenced::new(person_diane()))?; // Diane - kept
...
async fn test_filter_ordered_alternating_pattern() -> anyhow::Result<()> {
    ...
    // Act
    tx.try_send(Sequenced::new(person_alice()))?; // 1st person - kept
    tx.try_send(Sequenced::new(person_bob()))?; // 2nd person - filtered
    tx.try_send(Sequenced::new(animal_dog()))?; // not a person - filtered
    tx.try_send(Sequenced::new(person_charlie()))?; // 3rd person - kept
    tx.try_send(Sequenced::new(person_diane()))?; // 4th person - filtered
