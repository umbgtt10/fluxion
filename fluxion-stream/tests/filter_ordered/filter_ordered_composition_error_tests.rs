...
async fn test_distinct_until_changed_by_multiple_errors_in_composition() -> anyhow::Result<()> {
    ...
    // Act & Assert
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(1, 1)))?; // Odd
    assert!(matches!(
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(2, 5)))?; // Even
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(_)
    ));
...
async fn test_scan_ordered_error_propagation_with_filter() -> anyhow::Result<()> {
    ...
    // Act & Assert
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(100, 1)))?; // count=1, filtered
    assert_no_element_emitted(&mut result, 100).await;

    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(200, 2)))?; // count=2, emitted
    assert!(matches!(
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(300, 4)))?; // count=3, filtered
    assert_no_element_emitted(&mut result, 100).await;

    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(400, 5)))?; // count=4, emitted
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
