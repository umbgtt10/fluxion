...
async fn test_filter_ordered_propagates_errors() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(1, 1)))?;
    ...
    // Value filtered out
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(3, 3)))?;

    // Value that passes
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(4, 4)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
...
async fn test_filter_ordered_predicate_after_error() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(15, 2)))?;
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(20, 3)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(ref v) if v.value == 20
    ));

    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(25, 4)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
        StreamItem::Value(ref v) if v.value == 25
    ));

    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(30, 5)))?;
    assert!(matches!(
        unwrap_stream(&mut result, 100).await,
...
async fn test_filter_ordered_chain_with_map_after_error() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(10, 1)))?; // Filtered
    ...
    tx.try_send(StreamItem::Value(Sequenced::with_timestamp(30, 3)))?;
