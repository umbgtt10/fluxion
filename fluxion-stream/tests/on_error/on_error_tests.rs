...
async fn test_on_error_consumes_all_errors() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Error(FluxionError::stream_error("user error")))?;
...
async fn test_on_error_continues_after_consumed_error() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Value(Sequenced::new(4)))?;
    assert_eq!(
        unwrap_value(Some(unwrap_stream(&mut result, 100).await)).value,
        4
    );
...
async fn test_on_error_multiple_consecutive_errors() -> anyhow::Result<()> {
    ...
    // Act & Assert
    tx.try_send(StreamItem::Error(FluxionError::stream_error("user error1")))?;
    assert_no_element_emitted(&mut result, 100).await;

    tx.try_send(StreamItem::Error(FluxionError::stream_error("user error2")))?;
    assert_no_element_emitted(&mut result, 100).await;
    ...
    tx.try_send(StreamItem::Error(FluxionError::stream_error("user error4")))?;
    assert_no_element_emitted(&mut result, 100).await;

    tx.try_send(StreamItem::Value(Sequenced::new(99)))?;
    assert_eq!(
        unwrap_value(Some(unwrap_stream(&mut result, 100).await)).value,
        99
    );
...
async fn test_on_error_three_level_chain() -> anyhow::Result<()> {
    ...
    tx.try_send(StreamItem::Error(FluxionError::stream_error("other error")))?;
    assert_no_element_emitted(&mut result, 100).await;

    tx.try_send(StreamItem::Value(Sequenced::new(100)))?;
