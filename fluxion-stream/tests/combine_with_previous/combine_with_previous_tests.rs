...
async fn test_combine_with_previous_high_volume_sequential() -> anyhow::Result<()> {
    ...
    for i in 0..200 {
        match i % 3 {
            0 => tx.try_send(Sequenced::new(person_alice()))?,
            1 => tx.try_send(Sequenced::new(person_bob()))?,
            _ => tx.try_send(Sequenced::new(person_charlie()))?,
        }
    }
...
async fn test_combine_with_previous_boundary_empty_string_zero_values() -> anyhow::Result<()> {
    ...
    tx.try_send(Sequenced::new(person(String::new(), 0)))?;
...
async fn test_combine_with_previous_boundary_maximum_concurrent_streams() -> anyhow::Result<()> {
    ...
    for i in 0..num_concurrent {
        let handle = tokio::spawn(async move {
            ...
            tx.try_send(Sequenced::new(person(format!("Person{i}"), i)))
                .unwrap();
