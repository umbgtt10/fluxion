...
async fn test_take_latest_when_propagates_source_error() -> anyhow::Result<()> {
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(1, 1)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(2, 2)))?;

    // Send trigger
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 4)))?;
    assert!(matches!(
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 3)))?;
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(20, 5)))?;
    assert!(matches!(
        unwrap_stream(&mut triggered_stream, 500).await,
        StreamItem::Value(_)
    ));
...
async fn test_take_latest_when_propagates_trigger_error() -> anyhow::Result<()> {
    ...
    // Send trigger
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 3)))?;
    assert!(matches!(
        unwrap_stream(&mut triggered_stream, 500).await,
        StreamItem::Value(_)
    ));
    ...
    // Continue
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(30, 5)))?;
    assert!(matches!(
        unwrap_stream(&mut triggered_stream, 500).await,
        StreamItem::Value(_)
    ));
...
async fn test_take_latest_when_both_streams_have_errors() -> anyhow::Result<()> {
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(1, 1)))?;
    assert_no_element_emitted(&mut triggered_stream, 500).await;
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 2)))?;
    assert!(matches!(
        unwrap_stream(&mut triggered_stream, 500).await,
        StreamItem::Value(_)
    ));
...
async fn test_take_latest_when_trigger_before_source() -> anyhow::Result<()> {
    ...
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 1)))?;

    assert_no_element_emitted(&mut triggered_stream, 100).await;

    // Now send source value
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(42, 2)))?;

    // Still no emission (just cached)
    assert_no_element_emitted(&mut triggered_stream, 100).await;

    // Send another trigger - now should emit
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(20, 3)))?;
...
async fn test_take_latest_when_filter_returns_false() -> anyhow::Result<()> {
    ...
    // Setup source
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(100, 1)))?;
    ...
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(100, 3)))?;
...
async fn test_take_latest_when_multiple_triggers_no_source() -> anyhow::Result<()> {
    ...
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 1)))?;
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(20, 2)))?;
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(30, 3)))?;

    // No emissions should occur
    assert_no_element_emitted(&mut triggered_stream, 100).await;

    // Now send source value
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(999, 4)))?;

    // Still no emission (waiting for trigger)
    assert_no_element_emitted(&mut triggered_stream, 100).await;

    // Send one more trigger
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(40, 5)))?;
...
async fn test_take_latest_when_alternating_filter_conditions() -> anyhow::Result<()> {
    ...
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(5, 3)))?;
    assert_no_element_emitted(&mut triggered_stream, 100).await;

    // Trigger that passes (50 > 10)
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(50, 4)))?;
    assert!(matches!(
        unwrap_stream(&mut triggered_stream, 500).await,
        StreamItem::Value(_)
    ));
    ...
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(0, 5)))?;
    assert_no_element_emitted(&mut triggered_stream, 100).await;
...
async fn test_take_latest_when_source_updates_dont_emit() -> anyhow::Result<()> {
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(2, 2)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(3, 3)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(4, 4)))?;

    // None should emit
    assert_no_element_emitted(&mut triggered_stream, 100).await;

    // Send trigger - should emit latest (4)
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(10, 5)))?;
...
async fn test_take_latest_when_error_before_any_trigger() -> anyhow::Result<()> {
    ...
    // Continue normally
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(42, 1)))?;
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(1, 2)))?;
...
async fn test_take_latest_when_source_overwritten_between_triggers() -> anyhow::Result<()> {
    ...
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(200, 3)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(300, 4)))?;
    source_tx.send(StreamItem::Value(Sequenced::with_timestamp(400, 5)))?;

    // Trigger - should get latest (400)
    trigger_tx.send(StreamItem::Value(Sequenced::with_timestamp(2, 6)))?;
