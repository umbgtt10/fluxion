...
async fn test_combine_latest_take_latest_when() -> anyhow::Result<()> {
    ...
    trigger_person_tx.try_send(Sequenced::new(person_bob()))?;
    trigger_animal_tx.try_send(Sequenced::new(plant_rose()))?;
    ...
    trigger_person_tx.try_send(Sequenced::new(person_dave()))?;
...
async fn test_filter_ordered_take_latest_when() -> anyhow::Result<()> {
    ...
    source_tx.try_send(Sequenced::new(animal_dog()))?; // Filtered
    source_tx.try_send(Sequenced::new(person_bob()))?;

    filter_tx.try_send(Sequenced::new(person_alice()))?; // Trigger emission
    ...
    source_tx.try_send(Sequenced::new(plant_rose()))?; // Filtered

    filter_tx.try_send(Sequenced::new(person_bob()))?; // Trigger emission
    {
        let val = unwrap_value(Some(unwrap_stream(&mut stream, 500).await));
