...
async fn test_on_error_consumes_all_errors() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Error(FluxionError::stream_error("user error")))?;
...
async fn test_on_error_with_side_effects() -> anyhow::Result<()> {
    ...
    let (tx, stream) = test_channel_with_errors::<Sequenced<i32>>();
    let mut stream = FluxionStream::new(stream).on_error(move |err| {
...
async fn test_on_error_continues_after_consumed_error() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Value(Sequenced::new(4)))?;
    assert_eq!(
        unwrap_value(Some(unwrap_stream(&mut stream, 100).await)).value,
        4
    );
...
async fn test_on_error_multiple_consecutive_errors() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Error(FluxionError::stream_error("user error2")))?;
    assert_no_element_emitted(&mut stream, 100).await;
    ...
    tx.send(StreamItem::Error(FluxionError::stream_error("user error4")))?;
    assert_no_element_emitted(&mut stream, 100).await;

    tx.send(StreamItem::Value(Sequenced::new(99)))?;
    assert_eq!(
        unwrap_value(Some(unwrap_stream(&mut stream, 100).await)).value,
        99
    );
...
async fn test_on_error_three_level_chain() -> anyhow::Result<()> {
    ...
    tx.send(StreamItem::Error(FluxionError::stream_error("other error")))?;
    assert_no_element_emitted(&mut stream, 100).await;

    tx.send(StreamItem::Value(Sequenced::new(100)))?;
