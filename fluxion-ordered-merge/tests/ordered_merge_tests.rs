...
async fn test_ordered_merge_large_volume() -> anyhow::Result<()> {
    ...
    for _ in 0..500 {
        ...
        tx2.try_send(Sequenced::new(animal_dog()))?;
    }
...
async fn test_ordered_merge_maximum_concurrent_streams() -> anyhow::Result<()> {
    ...
    for _i in 0..num_concurrent {
        let handle = tokio::spawn(async move {
            ...
            tx3.try_send(Sequenced::new(plant_rose())).unwrap();
            ...
            // Act & Assert
            tx4.try_send(Sequenced::new(person_bob())).unwrap();
            assert_eq!(
                unwrap_value(Some(unwrap_stream(&mut results2, 100).await)).value,
                person_bob()
            );

            tx5.try_send(Sequenced::new(person_charlie())).unwrap();
            assert_eq!(
                unwrap_value(Some(unwrap_stream(&mut results2, 100).await)).value,
                person_charlie()
            );
