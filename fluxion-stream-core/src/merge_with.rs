...
//! Generic implementation of `MergedStream` - a stateful stream merger.
//!
...
/// A stateful stream merger that combines multiple Timestamped streams while maintaining state.
///
/// Internally uses [`ordered_merge_with_index`] to merge streams in order
    ...
    State: Send + 'static,
{
    ...
    /// # Example
    /// ```no_run
    /// # use fluxion_stream_core::MergedStream;
    ...
    <Item as HasTimestamp>::Timestamp: Debug + Ord + Send + Sync + Copy + 'static,
{
    ...
    ///
    /// Uses [`ordered_merge_with_index`] to combine the streams while preserving
