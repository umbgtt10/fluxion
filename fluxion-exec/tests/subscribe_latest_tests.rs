...
async fn test_subscribe_latest_no_skipping_no_error_no_cancellation() -> anyhow::Result<()> {
    ...
    spawn({
        async move {
            rx.subscribe_latest(func, error_callback, None)
                .await
    ...
    tx.try_send(Sequenced::new(person_diane()))?;
    ...
    tx.try_send(Sequenced::new(animal_ant()))?;
    ...
    assert_eq!(
        collected_items
            ...
            .await
            .iter()
            .map(|item| unwrap_value(Some(item.clone())).value)
            .collect::<Vec<_>>(),
        vec![
            person_alice(),
            person_bob(),
...
async fn test_subscribe_latest_with_skipping_no_error_no_cancellation() -> anyhow::Result<()> {
    ...
    let func = {
        ...
        move |stream_item, _| {
            ...
            async move {
                ...
                collected_items.lock().await.push(stream_item);
                let _ = notify_tx.try_send(());
                Ok::<(), TestError>(())
            }
        }
    ...
    tx.try_send(Sequenced::new(person_charlie()))?;
    tx.try_send(Sequenced::new(person_diane()))?;
    tx.try_send(Sequenced::new(person_dave()))?; // latest
...
async fn test_subscribe_latest_no_skipping_with_error_no_cancellation() -> anyhow::Result<()> {
    ...
    let func = {
        let collected_items = collected_items_clone.clone();
        let notify_tx = notify_tx.clone();
        move |stream_item: StreamItem<Sequenced<TestData>>, _| {
            let collected_items = collected_items.clone();
            let notify_tx = notify_tx.clone();
            async move {
                let item: Sequenced<TestData> = unwrap_value(Some(stream_item.clone()));
                let data = item.value;

                if matches!(&data, TestData::Person(p) if p.name == "Bob" || p.name == "Dave") {
                    let _ = notify_tx.try_send(());
                    return Err(TestError::new(format!("Failed to process {data:?}")));
                }
    ...
    tx.try_send(Sequenced::new(person_bob()))?; // Error
    notify_rx.next().await.expect("Bob handled (error)");

    tx.try_send(Sequenced::new(person_charlie()))?;
    notify_rx.next().await.expect("Charlie processed");

    tx.try_send(Sequenced::new(person_dave()))?; // Error
    ...
    */
    ...
    assert_eq!(
        collected_items
            ...
            .map(|item| unwrap_value(Some(item.clone())).value)
            .collect::<Vec<_>>(),
        vec![person_alice(), person_charlie(), animal_dog()]
    );
...
async fn test_subscribe_latest_no_skipping_no_errors_with_cancellation() -> anyhow::Result<()> {
    ...
    let func = {
        let collected_items = collected_items_clone.clone();
        let notify_tx = notify_tx.clone();
        move |stream_item, ctx: CancellationToken| {
            let collected_items = collected_items.clone();
            let notify_tx = notify_tx.clone();
            async move {
                if ctx.is_cancelled() {
    ...
    spawn({
        ...
        async move {
            rx.subscribe_latest(func, error_callback, Some(cancellation_token))
                .await
                .expect("subscribe_latest should succeed");
        }
    });
...
async fn test_subscribe_latest_no_skipping_with_cancellation_and_errors() -> anyhow::Result<()> {
    ...
    let func = {
        ...
        move |stream_item: StreamItem<Sequenced<TestData>>, _| {
            ...
            async move {
                ...
                // Error on animals
                if matches!(&data, TestData::Animal(_)) {
                    let _ = notify_tx.try_send(());
    ...
    tx.try_send(Sequenced::new(animal_dog()))?; // Error
...
async fn test_subscribe_latest_no_skipping_no_error_no_cancellation_no_concurrent_processing(
    ...
    let (started_tx, mut started_rx) = test_channel::<()>();
    let (finish_tx, finish_rx) = test_channel::<()>();
...
async fn test_subscribe_latest_no_skipping_no_error_no_cancellation_token_empty_stream(
    ...
    let func = move |stream_item, _| {
        let collected_items = collected_items_clone.clone();
        async move {
            let item: Sequenced<TestData> = unwrap_value(Some(stream_item));
            collected_items.lock().await.push(item.value);
            Ok::<(), TestError>(())
        }
    };
...
async fn test_subscribe_latest_high_volume() -> anyhow::Result<()> {
    ...
    let rx = rx.map(move |timestamped| {
    ...
    tx.try_send(Sequenced::new(person_bob()))?; // sentinel latest
...
async fn test_subscribe_latest_error_aggregation_with_callback() -> anyhow::Result<()> {
    ...
    let func = {
        let notify_tx = notify_tx.clone();
        move |stream_item: StreamItem<Sequenced<TestData>>, _ctx: CancellationToken| {
            let notify_tx = notify_tx.clone();
            async move {
                let _ = notify_tx.try_send(());
                let item: Sequenced<TestData> = unwrap_value(Some(stream_item));
                ...
                if matches!(&data, TestData::Animal(_)) {
                    Err(TestError::new(format!("Animals not allowed: {:?}", data)))
                } else {
                    Ok(())
                }
            }
    ...
    tx.try_send(Sequenced::new(animal_cat()))?; // Error
    notify_rx.next().await.unwrap();
