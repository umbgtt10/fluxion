...
/// Benchmark for ordered combine_latest using ordered_merge.
///
/// This variant maintains temporal ordering based on timestamps.
///
/// # Panics
///
/// This benchmark constructs a local `Runtime` with `Runtime::new().unwrap()`, which may panic.
pub fn bench_combine_latest_ordered(c: &mut Criterion) {
    let mut group = c.benchmark_group("combine_latest_ordered");
...
criterion_group!(benches, bench_combine_latest_ordered);
