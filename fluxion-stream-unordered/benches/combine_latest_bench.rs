...
/// Benchmark for unordered combine_latest using select_all.
///
/// This variant processes events in arrival order for better performance.
///
/// # Panics
///
/// This benchmark constructs a local `Runtime` with `Runtime::new().unwrap()`, which may panic.
pub fn bench_combine_latest_unordered(c: &mut Criterion) {
    let mut group = c.benchmark_group("combine_latest_unordered");
...
criterion_group!(benches, bench_combine_latest_unordered);
